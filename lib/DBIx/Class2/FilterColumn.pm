package DBIx::Class2::FilterColumn;
use strict;
use warnings;
...
sub filter_column {
  ...
  $self->throw_exception("FilterColumn can not be used on a column with a declared InflateColumn inflator")
    if defined $colinfo->{_inflate_info} and $self->isa('DBIx::Class2::InflateColumn');
...
sub new {
  ...
  my $rsrc = $data->{-result_source}
    or $class->throw_exception('Sourceless rows are not supported with DBIx::Class2::FilterColumn');
...
DBIx::Class2::FilterColumn - Automatically convert column data
...
L<DBIx::Class2::InflateColumn>.  One of the major issues with said component is
that it B<only> works with references.  Generally speaking anything that can
be done with L<DBIx::Class2::InflateColumn> can be done with this component.
