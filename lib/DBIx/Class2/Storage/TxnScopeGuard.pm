package DBIx::Class2::Storage::TxnScopeGuard;
...
use DBIx::Class2;
use DBIx::Class2::_Util qw(is_exception detected_reinvoked_destructor);
...
sub DESTROY {
  ...
  $self->{storage}->_verify_pid unless DBIx::Class2::_ENV_::BROKEN_FORK;
...

  {
    ...
    carp 'A DBIx::Class2::Storage::TxnScopeGuard went out of scope without explicit commit or error. Rolling back.'
    ...
    if ( $rollback_exception and (
      ! defined blessed $rollback_exception
          or
      ! $rollback_exception->isa('DBIx::Class2::Storage::NESTED_ROLLBACK_EXCEPTION')
    ) ) {
      # append our text - THIS IS A TEMPORARY FIXUP!
      # a real stackable exception object is in the works
      if (ref $exception eq 'DBIx::Class2::Exception') {
...
DBIx::Class2::Storage::TxnScopeGuard - Scope-based transaction handling
...
right thing with transactions in DBIx::Class2.
...
implicitly calling L<DBIx::Class2::Storage/txn_begin>. Expects a
L<DBIx::Class2::Storage> object as its only argument.
...
the transaction is rolled back, via L<DBIx::Class2::Storage/txn_rollback>

=cut

=head1 SEE ALSO

L<DBIx::Class2::Schema/txn_scope_guard>.
