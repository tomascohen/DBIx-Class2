package DBIx::Class2::Storage::DBI::Cursor;

use strict;
use warnings;

use base 'DBIx::Class2::Cursor';
...
DBIx::Class2::Storage::DBI::Cursor - Object representing a query cursor on a
resultset.
...
A Cursor represents a query cursor on a L<DBIx::Class2::ResultSet> object. It
allows for traversing the result set with L</next>, retrieving all results with
L</all> and resetting the cursor with L</reset>.

Usually, you would use the cursor methods built into L<DBIx::Class2::ResultSet>
to traverse it. See L<DBIx::Class2::ResultSet/next>,
L<DBIx::Class2::ResultSet/reset> and L<DBIx::Class2::ResultSet/all> for more
information.
...
Returns a new L<DBIx::Class2::Storage::DBI::Cursor> object.
...

{
  ...
  sub new {
    ...
    if (DBIx::Class2::_ENV_::HAS_ITHREADS) {
...
sub all {
  ...
  (
    DBIx::Class2::_ENV_::SHUFFLE_UNORDERED_RESULTSETS
      and
    ! $self->{attrs}{order_by}
      and
    require List::Util
...
sub sth {
  ...
  if (@_) {
    ...
    $self->{sth} = $_[0];
    $self->{_pid} = $$ if ! DBIx::Class2::_ENV_::BROKEN_FORK and $_[0];
  }
  elsif ($self->{sth} and ! $self->{_done}) {

    my $invalidate_handle_reason;

    if (DBIx::Class2::_ENV_::HAS_ITHREADS and $self->{_intra_thread} ) {
      $invalidate_handle_reason = 'Multi-thread';
    }
    elsif (!DBIx::Class2::_ENV_::BROKEN_FORK and $self->{_pid} != $$ ) {
...
Resets the cursor to the beginning of the L<DBIx::Class2::ResultSet>.
