package # hide from PAUSE
    DBIx::Class2::CDBICompat::Relationships;
...
use Clone;
use DBIx::Class2::CDBICompat::Relationship;
...
DBIx::Class2::CDBICompat::Relationships - Emulate has_a(), has_many(), might_have() and meta_info()
...
sub _declare_has_a {
  ...
  if( !$f_class->isa('DBIx::Class2::Row') and !$f_class->isa('Class::DBI::Row') ) {
...
sub _extend_meta {
    ...
    $hash{$type}{$rel} = DBIx::Class2::CDBICompat::Relationship->new($val);
...
sub search {
  ...
  if (ref $where eq 'HASH') {
    foreach my $key (keys %$where) { # has_a deflation hack
      $where->{$key} = ''.$where->{$key}
        if eval { $where->{$key}->isa('DBIx::Class2') };
    }
  }
