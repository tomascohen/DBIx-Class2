...
use DBIx::Class2::Carp '^DBIx::Class2|^Class::C3::Componentised';
use namespace::clean;
...
sub inject_base {
  ...
  my $keep_checking = ! (
    $target->isa ('DBIx::Class2::UTF8Columns')
      ||
    $target->isa ('DBIx::Class2::ForceUTF8')
  );
  ...
  while ($keep_checking && @complist) {
    ...
    for (qw/DBIx::Class2::UTF8Columns DBIx::Class2::ForceUTF8/) {
      if ($comp->isa ($_) ) {
        $keep_checking = 0; # no use to check from this point on
        carp_once "Use of $_ is strongly discouraged. See documentation of DBIx::Class2::UTF8Columns for more info\n"
    ...
    if (! $keep_checking) {
      ...
      my $base_store_column = do { require DBIx::Class2::Row; DBIx::Class2::Row->can ('store_column') };
      ...
      carp "Incorrect loading order of $comp by $target will affect other components overriding 'store_column' ("
          . join (', ', @broken)
          .'). Refer to the documentation of DBIx::Class2::UTF8Columns for more info'
        if @broken;
    }
