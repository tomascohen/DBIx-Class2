...
DBIx::Class2::Manual::FAQ - Frequently Asked Questions (in theory)

=head1 DESCRIPTION

This document is intended as an anti-map of the documentation. If you
know what you want to do, but not how to do it in L<DBIx::Class2>, then
...
DBIx::Class2 schema. If it's the former, look up the documentation for
...
=item .. use DBIx::Class2 with L<Catalyst>?

Install L<Catalyst::Model::DBIC::Schema> from CPAN. See its
documentation, or below, for further details.

=item .. set up my DBIx::Class2 classes automatically from my database?

Install L<DBIx::Class2::Schema::Loader> from CPAN, and read its documentation.

=item .. set up my DBIx::Class2 classes manually?

Look at the L<DBIx::Class2::Manual::Example> and come back here if you get lost.

=item .. create my database tables from my DBIx::Class2 schema?

Create your classes manually, as above. Write a script that calls
L<DBIx::Class2::Schema/deploy>. See there for details, or the
...
L<DBIx::Class2::Manual::Cookbook/Using Unicode>

=item .. connect to my database?

Once you have created all the appropriate table/source classes, and an
overall L<Schema|DBIx::Class2::Schema> class, you can start using
...
tables. See L<DBIx::Class2::Schema/connect> for details. The actual
...
=item .. use DBIx::Class2 across multiple databases?

If your database server allows you to run queries across multiple
databases at once, then so can DBIx::Class2. All you need to do is make
sure you write the database name as part of the
L<table|DBIx::Class2::ResultSourceProxy::Table/table> call. Eg:

  __PACKAGE__->table('mydb.mytablename');

And load all the Result classes for both / all databases by calling
L<DBIx::Class2::Schema/load_namespaces>.

=item .. use DBIx::Class2 across PostgreSQL/DB2/Oracle schemas?

Add the name of the schema to the table name, when invoking
L<table|DBIx::Class2::ResultSourceProxy::Table/table>, and make sure the user
...
=item .. tell DBIx::Class2 about relationships between my tables?

There are a variety of relationship types that come pre-defined for
you to use.  These are all listed in L<DBIx::Class2::Relationship>. If
...
be set up individually on each side. See L<DBIx::Class2::Relationship>
for details.
...
foreign key.  See L<DBIx::Class2::Relationship/belongs_to>.
...
as you like. See L<DBIx::Class2::Relationship::Base>.
...
relationship in the fullest sense.  For more info, read the documentation on L<DBIx::Class2::Relationship/many_to_many>.

=item .. stop DBIx::Class2 from attempting to cascade deletes on my has_many and might_have relationships?

By default, DBIx::Class2 cascades deletes and updates across
...
database?>. Find the
L<ResultSet|DBIx::Class2::Manual::Glossary/ResultSet> that you want to
search in, by calling C<< $schema->resultset('MySource') >> and call
C<search> on it. See L<DBIx::Class2::ResultSet/search>.
...
relationship between the two tables, see L<DBIx::Class2::Relationship>
for the join used by each relationship.

=item .. create joins with conditions other than column equality?

Currently, L<DBIx::Class2> can only create join conditions using
...
Behind the scenes, DBIx::Class2 uses L<SQL::Abstract::Classic> to help construct
its SQL searches. So if you fail to find help in the
L<DBIx::Class2::Manual::Cookbook>, try looking in the L<SQL::Abstract::Classic>
documentation.
...
L<search|DBIx::Class2::ResultSet/search> and L<find|DBIx::Class2::ResultSet/find>
do not take L<DBIx::Class2::InflateColumn> into account, and so your L<DateTime>
object will not be correctly deflated into a format your RDBMS expects.

The L<datetime_parser|DBIx::Class2::Storage::DBI/datetime_parser> method on your
...
L<C<result_source>|DBIx::Class2::Row/result_source>.
...
whereas L<create|DBIx::Class2::ResultSet/create> and L<DBIx::Class2::Row/update>
(but not L<DBIx::Class2::ResultSet/update>) are
...
See the prefetch examples in the L<Cookbook|DBIx::Class2::Manual::Cookbook>.

=item .. fetch a whole column of data instead of a row?

Call C<get_column> on a L<DBIx::Class2::ResultSet>. This returns a
L<DBIx::Class2::ResultSetColumn>. See its documentation and the
L<Cookbook|DBIx::Class2::Manual::Cookbook> for details.
...
See the L<Cookbook|DBIx::Class2::Manual::Cookbook> for more details.

=item .. fetch a single (or topmost) row?

Use the L<DBIx::Class2::ResultSet/rows> and
L<DBIx::Class2::ResultSet/order_by> attributes to order your data and
pick off a single row.

See also L<DBIx::Class2::Manual::Cookbook/Retrieve one and only one row from a resultset>.

A less readable way is to ask a regular search to return 1 row, using
L<DBIx::Class2::ResultSet/slice>:
...
speed improvement. The method using L<DBIx::Class2::ResultSet/single> mentioned
in the cookbook can do the same if you pass a C<rows> attribute to the search.

=item .. refresh a row from storage?

Use L<DBIx::Class2::Row/discard_changes>.
...
L<DBIx::Class2::Row/discard_changes> does just that by re-fetching the row from storage
...
|DBIx::Class2::ResultSet::Pager> with an API/behavior identical to that
...
L<creating|DBIx::Class2::ResultSet/create> a result object, the primary
...
The C<populate> method in L<DBIx::Class2::ResultSet> provides
efficient bulk inserts.

L<DBIx::Class2::Fixtures> provides an alternative way to do this.
...
You can use L<DBIx::Class2::InflateColumn> to accomplish YAML/JSON storage transparently.
...
When two tables ARE related, L<DBIx::Class2::Relationship::Base> provides many
...
what create_related() from L<DBIx::Class2::Relationship::Base> does, you could
add this to Book.pm:
...
=item How do I store my own (non-db) data in my DBIx::Class2 objects?
...
And another method is to use L<Moose> with your L<DBIx::Class2> package.
	...
	extends 'DBIx::Class2::Core'; # Moose changes the way we define our parent (base) package
...
=item How do I use DBIx::Class2 objects in my TT templates?
...
L<DBIx::Class2> runs the actual SQL statement as late as possible, thus
...
If your table lacks a primary key, DBIx::Class2 can't work out which row
it should operate on, for example to delete or update.  However, a
UNIQUE constraint on one or more columns allows DBIx::Class2 to uniquely
identify the row, so you can tell L<DBIx::Class2::ResultSource> these
...
Look at the tips in L<DBIx::Class2::Manual::Cookbook/"STARTUP SPEED">

=item How do I reduce the overhead of database queries?

You can reduce the overhead of object creation within L<DBIx::Class2>
using the tips in L<DBIx::Class2::Manual::Cookbook/"Skip result object creation for faster results">
and L<DBIx::Class2::Manual::Cookbook/"Get raw data for blindingly fast results">
...
See L<DBIx::Class2::Manual::Cookbook/Stringification>
