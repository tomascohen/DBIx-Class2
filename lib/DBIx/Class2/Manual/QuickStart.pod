...
DBIx::Class2::Manual::QuickStart - up and running with DBIC in 10 minutes
...
First, install DBIx::Class2 like you do with any other CPAN distribution.
...
mentioned earlier, the next command will download and unpack it:
    ...
    $ perl -mCPAN -e'CPAN::Shell->look("DBIx::Class2")'
    DBIx-Class2$ cd examples/Schema

Inspect the database:

    DBIx-Class2/examples/Schema$ sqlite3 db/example.db .dump
...
to the tables in the database. L<DBIx::Class2::Manual::Example> shows how to
write all that Perl code. That is almost never necessary, though. Instead use
L<dbicdump> (part of the distribution L<DBIx::Class2::Schema::Loader>) to
...
A L<schema|DBIx::Class2::Manual::Glossary/Schema> object represents the database.
...
L<resultset|DBIx::Class2::Manual::Glossary/ResultSet> object.
...
L<Result|DBIx::Class2::Manual::Glossary/Result> objects represent a row and
...
=head2 Resetting the database
    ...
    DBIx-Class2/examples/Schema$ rm -f db/example.db

    # create database and set up tables from definition
    DBIx-Class2/examples/Schema$ sqlite3 db/example.db < db/example.sql

    # fill them with data
    DBIx-Class2/examples/Schema$ perl ./insertdb.pl

    # delete the schema classes files
    DBIx-Class2/examples/Schema$ rm -rf MyApp

    # recreate schema classes files from database file
    DBIx-Class2/examples/Schema$ dbicdump \
...
L<DBIx::Class2::Manual::SQLHackers>.

Continue with L<DBIx::Class2::Tutorial> and
L<DBIx::Class2/"WHERE TO START READING">.
