...
use DBIx::Class2::Carp '^DBIx::Class2|^DBICTest';
...
sub scope_guard (&) {
  ...
  bless [ $_[0] ], 'DBIx::Class2::_Util::ScopeGuard';
}
{
  package #
    DBIx::Class2::_Util::ScopeGuard;

  sub DESTROY {
    &DBIx::Class2::_Util::detected_reinvoked_destructor;

    local $@ if DBIx::Class2::_ENV_::UNSTABLE_DOLLARAT;
...
sub is_exception ($) {
  ...
  if (defined $suberror) {
    if (length (my $class = blessed($e) )) {
      ...
      . '"exception substitution". DBIx::Class2 tries to work around this '
  ...
  sub fail_on_internal_wantarray () {
    ...
    my ($fr, $want, $argdesc);
    {
      ...
      $argdesc = ref $DB::args[0]
        ? DBIx::Class2::_Util::refdesc($DB::args[0])
        : 'non '
      ;
    };

    if (
      $want and $fr->[0] =~ /^(?:DBIx::Class2|DBICx::)/
    ) {
      DBIx::Class2::Exception->throw( sprintf (
